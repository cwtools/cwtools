...
and Node (key : string, pos : range) =
    ...
    member this.AllArray with get() = all
    member this.AllArray with set(value : Child array) = all <- value; reset()
...
module ProcessCore =
    ...
    let processNode< 'T when 'T :> Node > (postinit : 'T -> 'T) inner (key : string) (pos : range) (sl : Statement list) : Node =
        ...
        node.All <- sl |> List.map (fun e -> inner e)
        node
    ...
    type BaseProcess (maps : NodeTypeMap ) =
        let rec lookup =
            (fun (key : string) (pos : range) (context : LookupContext) ->
                ...
                ) >> (fun f a b c -> NodeC (f a b c)) //|> (fun n -> n.AllArray <- n.AllArray |> Array.rev; n)))
        and processNodeInner (c : LookupContext) statement : Child =
            //eprintfn "%A" node.Key

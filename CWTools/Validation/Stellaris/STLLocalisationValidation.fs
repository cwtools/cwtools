...
module STLLocalisationValidation =
    ...
    let checkLocKey (leaf : Leaf) (keys : Set<string>) (lang : Lang) key =
        ...
        | true, _ -> OK
        | _, true -> OK
        | _, false -> Invalid (seq {yield invData (ErrorCodes.MissingLocalisation key (lang)) leaf (Some key)})
    ...
    let checkLocLeafValue (keys : Set<string>) (lang : Lang) key (lv : LeafValue) =
        if lang = STL STLLang.Default then OK else
            ...
            | true, _ -> OK
            | _, true -> OK
            | _, false -> Invalid (seq {yield invLeafValue (ErrorCodes.MissingLocalisation key (lang)) lv (Some key)})
    ...
    let checkLocNode (keys : Set<string>) (lang : Lang) key (node : Node) =
        if lang = STL STLLang.Default then OK else
            ...
            | true, _ -> OK
            | _, true -> OK
            | _, false -> Invalid (seq {yield invData (ErrorCodes.MissingLocalisation key (lang)) node (Some key)})

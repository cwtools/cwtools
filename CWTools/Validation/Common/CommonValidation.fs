...
module CommonValidation =
    ...
    let valInlineScriptParams<'T when 'T :> ComputedData> : CWTools.Games.LookupFileValidator<'T> =
        (fun fileManager rulesValidator lu res es ->
            ...
            | Some rv, Some ses ->
                let allInlineScripts = ses |> List.map (fun se -> se.id, se.range.FileName, findIS se.range)
                let getRefsFromRefTypes (referencedtypes: Map<string, ReferenceDetails list>) =
                    //eprintfn "grfrt %A" referencedtypes
                    referencedtypes |> (fun refMap -> Map.tryFind "inline_script" refMap) |> Option.defaultValue []
                let allRefs = es.AllWithData |> List.collect (fun (n, d) -> d.Force().Referencedtypes |> Option.map getRefsFromRefTypes |> Option.defaultValue [])
                                             |> List.filter (fun ref -> ref.referenceType = ReferenceType.TypeDef)
                                             |> List.map (fun ref -> {| scriptName = ref.name; callSite = ref.position; isParams = findParams ref.position ref.name|})
                ...
                let validateIS (name: string) (logicalpath : string) (node : Node option) (refs : {|callSite : range ; scriptName : string ; isParams : (string * string) list option|} list) =
                    ...
                    let res =
                        match node with
                        | Some node ->
                            refs <&!&> (fun ref -> memoizeValidation (name, logicalpath, node, ref.callSite, ref.isParams))
                        | None -> OK
                    res

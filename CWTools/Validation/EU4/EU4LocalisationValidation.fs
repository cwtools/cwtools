...
module EU4LocalisationValidation =
    type LocalisationValidator =
        EntitySet<EU4ComputedData> -> (Lang * Set<string>) list -> EntitySet<EU4ComputedData> -> ValidationResult
    ...
    let valEventModifierLocs: LocalisationValidator =
        fun _ keys es ->
            ...
            event_modifier <&!&> inner1 <&&> (event_modifier <&!&> inner2)
    ...
    let valTriggeredModifierLocs: LocalisationValidator =
        fun _ keys es ->
            ...
            let inner1 = checkLocNodeKeyAdvs keys "" [ "" ]
            let inner2 = checkLocNodeKeyAdvs keys "desc_" [ "" ]
            triggered_modifier <&!&> inner1 <&&> (triggered_modifier <&!&> inner2)

    let valProvinceTriggeredModifierLocs: LocalisationValidator =
        fun _ keys es ->
            let province_triggered_modifier =
                es.GlobMatchChildren("**/common/province_triggered_modifiers/*.txt")
    ...
    let valTradeCompanyInvestmentLocs: LocalisationValidator =
        fun _ keys es ->
            let trade_company_investment =
                es.GlobMatchChildren("**/common/tradecompany_investments/*.txt")

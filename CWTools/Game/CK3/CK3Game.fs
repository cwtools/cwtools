...
module CK3GameFunctions =
    ...
    let addScriptFormulaLinks (lookup : Lookup) =
        ...
        | Some vs ->
            ...
            values |> List.map (fun v -> Effect(v, [scopeManager.AnyScope], EffectType.ValueTrigger, Some (TypeRef("script_value", v))))
        | None -> []
    ...
    let createEmbeddedSettings embeddedFiles cachedResourceData (configs : (string * string) list) cachedRuleMetadata =
        ...
        let irEffects =
            configs |> List.tryFind (fun (fn, _) -> Path.GetFileName fn = "effects.log")
                    ...
                    |> Option.defaultWith (fun () -> eprintfn "effects.log was not found in ck3 config"; ([]))
        ...
        let irTriggers =
            configs |> List.tryFind (fun (fn, _) -> Path.GetFileName fn = "triggers.log")
                    ...
                    |> Option.defaultWith (fun () -> eprintfn "triggers.log was not found in ck3 config"; ([]))
        let featureSettings =
...
type CK3Game(setupSettings : CK3Settings) =
    ...
    let changeScope = Scopes.createJominiChangeScope CWTools.Process.Scopes.CK3.oneToOneScopes (Scopes.complexVarPrefixFun "variable:from:" "variable:")
    ...
    interface IGame<JominiComputedData> with
        ...
        member __.GoToType pos file text = getInfoAtPos fileManager game.ResourceManager game.InfoService game.LocalisationManager lookup (CK3 CK3Lang.English) pos file text

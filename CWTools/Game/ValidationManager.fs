...
// type ThirdPartyErrorCache() =
//
//     let fileToErrors = ConcurrentDictionary<Entity, ConcurrentDictionary<string, CWError list>>()
//     let errorToFile = ConcurrentDictionary<Entity, ConcurrentBag<CWError>>()
//
//     member this.AddErrorsGeneratedByFile (fromFile : string, errorsForFiles : Dictionary<string, CWError list>) =
//         // Remove existing errors generated by this file
//         if fileToErrors.ContainsKey fromFile then
//             match fileToErrors.TryRemove fromFile with
//             | (true, oldErrors) -> 
//                 for pair in oldErrors do
//                     let targetFile, errors = pair.Key, pair.Value
//                     let bag = errorToFile.[targetFile]
//                     for error in errors do
//                         bag.TryTake error |> ignore
//             | _ -> ()
//
//         // Add new errors
//         let newErrors = new ConcurrentDictionary<string, CWError list>(errorsForFiles)
//         fileToErrors.[fromFile] <- newErrors
//
//         for pair in errorsForFiles do
//             let targetFile, errors = pair.Key, pair.Value
//             let bag = errorToFile.GetOrAdd(targetFile, fun _ -> new ConcurrentBag<CWError>())
//             for error in errors do
//                 bag.Add error
//
//     member this.GetErrorsForFile (file : string) =
//         match errorToFile.TryGetValue(file) with
...
type ValidationManager<'T when 'T :> ComputedData>
    ...
    let validate (shallow : bool) (entities : struct (Entity * Lazy<'T>) list) =
        ...
        log $"Validation cache size %i{errorCache.Count}"

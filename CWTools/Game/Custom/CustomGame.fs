...
module CustomGameFunctions =
    ...
    let updateScriptedTriggers (lookup : Lookup<Scope, CustomModifier>) (rules :RootRule<Scope> list) (embeddedSettings : EmbeddedSettings<_,_>) =
        ...
        let extraFromRules = (effects |> List.choose ruleToTrigger |> List.map (fun e -> e :> Effect<Scope>))
    ...
    let addModifiersAsTypes (lookup : Lookup<Scope, CustomModifier>) (typesMap : Map<string,(bool * string * range) list>) =
    ...
    let addTriggerDocsScopes (lookup : Lookup<Scope, CustomModifier>) (rules : RootRule<_> list) =
            ...
            let addRequiredScopesE (s : StringTokens) (o : Options<_>) =
                ...
                let innerScope =
                    ...
                    | None ->
                        lookup.effectsMap.TryFind((StringResource.stringManager.GetStringForID s.normal))
                            |> Option.bind (function | :? DocEffect<Scope> as se -> Some se | _ -> None)
...
type CustomGame(setupSettings : CustomSettings, gameFolderName : string) =
    ...
    interface IGame<ComputedData, Scope, CustomModifier> with

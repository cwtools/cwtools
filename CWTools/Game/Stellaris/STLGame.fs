...
module STLGameFunctions =
    ...
    let addTriggerDocsScopes (lookup : Lookup) (rules : RootRule list) =
            let scriptedOptions (effect : ScriptedEffect) =
                {min = 0; max = 100; leafvalue = false; description = Some effect.Comments; pushScope = None; replaceScopes = None; severity = None; requiredScopes = effect.Scopes; comparison = false; referenceDetails = None; sourceFileLocation = None}

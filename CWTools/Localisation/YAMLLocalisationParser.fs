...
module YAMLLocalisationParser =
    ...
    let desc =
        many1Satisfy isLocValueChar .>> spaces <?> "desc"
    ...
    let file =
        ...
        >>. pipe2 (key)
                (many
                    ((attempt comment |>> (fun _ -> None))
                     <|> (entry |>> Some))
                 .>> eof) (fun k es -> { key = k; entries = List.choose id es })
        <?> "file"
    ...
    type YAMLLocalisationService<'L>(files: (string * string) list, keyToLanguage, gameLang) =
        ...
        let addFile f t =
            ...
            | Success ({ key = key; entries = entries }, _, _) ->
                ...
                | Some l ->
                    let es =
                        entries
                        |> List.map (fun e -> struct (e, gameLang l))
        ...
        new(localisationSettings: LocalisationSettings<'L>) =
            ...
            | true ->
                ...
                let rootFiles =
                    Directory.EnumerateFiles localisationSettings.folder
                    |> List.ofSeq
                ...
                YAMLLocalisationService
                    (actualFiles, localisationSettings.keyToLanguage, localisationSettings.gameToLang)
            | false ->

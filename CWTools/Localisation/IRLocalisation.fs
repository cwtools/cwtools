...
module IRLocalisation =
    open YAMLLocalisationParser
    open FParsec

    type IRLocalisationService(files : (string * string) list) =
        ...
        let addFile f t =
            ...
            | Success({key = key; entries = entries}, _, _) ->
                ...
                |Some l ->
                    let es = entries |> List.map (fun e -> struct (e, IR l))
        ...
        new(localisationSettings : LocalisationSettings) =
            log (sprintf "Loading IR localisation in %s" localisationSettings.folder)
            ...
            | true ->
                        ...
                        IRLocalisationService(actualFiles)
            | false ->
                log (sprintf "%s not found" localisationSettings.folder)
                IRLocalisationService([])
        member __.Api lang = {

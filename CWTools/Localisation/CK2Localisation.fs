...
module CK2Localisation =
    ...
    let escapeChar = skipChar '\\' >>. anyOf "\"\\/bfnrt,"
                    |>> function
                        | 'b' -> '\b'
                        | 'f' -> '\u000C'
                        | 'n' -> '\n'
    ...
    type CK2LocalisationService(files : (string * string) list) =
        ...
        let addFile (filename, filetext : string) =
            ...
            let fileRows =
                ...
                //|> String.concat "\n"
            // try
            let rows = fileRows |> List.map (parseRow filename) |> List.choose (function |Success (r, _,_) -> Some r |_ -> None) //LocalisationEntry.ParseRows file |> List.ofSeq
            let rows = rows |> List.mapi (fun i r -> makeRangeForRow i filename, r)
            csv <- rows @ csv
        ...
        let getKeys() = csv |> Seq.map (snd >> (fun f -> f.Code)) |> List.ofSeq
        let valueMap lang =
            ...
            one |> Seq.map (fun (p, k, v) -> k, {key = k; value = None; desc = v; position = p })
                                  |> Map.ofSeq
        let values lang =

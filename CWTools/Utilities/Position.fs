...
let posOrder   = orderOn (fun (p:pos) -> p.Line, p.Column) (porder (Int32.order, Int32.order))
(* rangeOrder: not a total order, but enough to sort on ranges *)
let rangeOrder = orderOn (fun (r:range) -> r.FileName, r.Start) (porder (String.order, posOrder))

...
internal class Program
{
    private static void main(string[] args)
    {
        ...
        var parsed2 = Parsers.ParseScriptFile("testevent2.txt", text2);
        ...
        var parsed = Parsers.ParseScriptFile("testevent.txt", text);

        var eventFile = parsed.GetResult();

        //"Process" result into nicer format
        // var processed = CK2Process.processEventFile(eventFile);
        var processed = Parsers.ProcessStatements("testevent.txt", "./testevent.txt", eventFile);
        ...
        var leaf = new Leaf(
            KeyValueItem.NewKeyValueItem(Key.NewKey("is_triggered_only"), Value.NewBool(true), Operator.Equals),
            FSharpOption<Range>.None);
        ...
        var folders = Utils.getAllFoldersUnion(new List<string> { @"C:\Users\Thomas\Git\cwtools-stellaris-config" });
        ...
        var (rules, types, enums, complexenums, values) =
            Helpers.LoadAndInitializeFromConfigFiles(files, Game.STL, false, false);
        var typeInfo = Helpers.GetTypesInFile("events/testevent.txt", processed, types);
        PrintfModule
            .PrintFormatLine(
                new PrintfFormat<
                    FSharpFunc<IReadOnlyDictionary<string, IReadOnlyCollection<NewScope.TypeDefInfo>>, Unit>, TextWriter
                    , Unit, Unit, IReadOnlyDictionary<string, IReadOnlyCollection<NewScope.TypeDefInfo>>>("%A"))
            .Invoke(typeInfo);

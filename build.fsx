...
let release = File.read "RELEASE_NOTES.md" |> Fake.Core.ReleaseNotes.parse
...
Core.Target.create "Clean" (fun _ ->
    ...
    |> Seq.iter (fun p ->
        ...
        DotNet.exec ((fun (b: DotNet.Options) -> { b with WorkingDirectory = dir })) "clean" ""
        |> ignore)

    DotNet.exec id "clean" "" |> ignore

    !!"**/obj/**/*.nuspec" |> File.deleteAll
...
Core.Target.create "Test" (fun _ ->
    ...
    |> Seq.iter (fun p ->
        ...
        DotNet.exec ((fun (b: DotNet.Options) -> { b with WorkingDirectory = dir })) "run" ""
        |> ignore))
...
Core.Target.create "PublishTool" (fun _ ->
    ...
    DotNet.exec
        (fun o ->
            { o with
                WorkingDirectory = "./CWToolsCLI/output" })
        "nuget"
        (sprintf " push *.nupkg -k %s -s https://api.nuget.org/v3/index.json" token)
    |> ignore)
...
"Clean" ==> "Restore" ==> "Build" ==> "PackageTool" ==> "PublishTool"

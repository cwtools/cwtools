...
let perf (b) =
    ...
    let configFiles =
        (if Directory.Exists "./testfiles/performancetest2/.cwtools" then
             getAllFoldersUnion ([ "./testfiles/performancetest2/.cwtools" ] |> Seq.ofList)
         else
             Seq.empty)
...
let perf3 (b) =
    ...
    let configFiles =
        (if Directory.Exists @"C:\Users\Thomas\git\cwtools-eu4-config\" then
             getAllFoldersUnion ([ @"C:\Users\Thomas\git\cwtools-eu4-config\" ] |> Seq.ofList)
         else
             Seq.empty)
        |> Seq.collect (Directory.EnumerateFiles)
    ...
    let settings =
        { CWTools.Games.EU4.EU4Settings.rootDirectories =
            [ WD
                  { WorkspaceDirectory.name = "Europa Universalis IV"
                    path = @"D:\Games\Steam\steamapps\common\Europa Universalis IV" } ]
          modFilter = None
...
let perf4 (b) =
    ...
    let configFiles =
        (if Directory.Exists @".\testfiles/custom/rules/" then
             getAllFoldersUnion ([ @".\testfiles/custom/rules" ] |> Seq.ofList)
         else
             Seq.empty)
        |> Seq.collect (Directory.EnumerateFiles)
    ...
    let settings =
        { CWTools.Games.EU4.EU4Settings.rootDirectories =
            [ WD
                  { WorkspaceDirectory.name = "Test"
                    path = @"./testfiles/custom/files" } ]
          modFilter = None
          scriptFolders = folders
          excludeGlobPatterns = None
          validation =
...
let perf5 (b) =
    ...
    let configFiles =
        (if Directory.Exists @"C:\Users\Thomas\git\cwtools-ck3-config\" then
             getAllFoldersUnion ([ @"C:\Users\Thomas\git\cwtools-ck3-config\" ] |> Seq.ofList)
         else
             Seq.empty)
        |> Seq.collect (Directory.EnumerateFiles)
    ...
    let settings =
        { CWTools.Games.CK3.CK3Settings.rootDirectories =
            [ WD
                  { WorkspaceDirectory.name = "Crusader Kings III"
                    path = @"D:\Games\Steam\steamapps\common\Crusader Kings III\game" } ]
          modFilter = None
          scriptFolders = folders
          excludeGlobPatterns = None
          validation =
...
let perfSTL (b) =
    ...
    let cached, cachedFiles =
        Serializer.deserialize @"C:\Users\Thomas\Git\cwtools-vscode\.cwtools\stl.cwb"

    let configs =
        CWToolsCLI.getConfigFiles (None, Some @"C:\Users\Thomas\Git\cwtools-stellaris-config\config")
...
let perfHOI4Vanilla (b) =
    ...
    let hoi4settings =
        { rootDirectories =
            [ WorkspaceDirectoryInput.WD
                  { path = @"D:\Games\Steam\steamapps\common\Hearts of Iron IV"
                    name = "test" } ]
          scriptFolders = folders
          excludeGlobPatterns = None
          embedded = FromConfig([], [])
...
let perfHOI4 (b) =
    ...
    let hoi4settings =
        { rootDirectories =
            [ WorkspaceDirectoryInput.WD
                  { path = @"D:\Synced\Git\Third Party\Hearts of Iron IV\Millennium_Dawn"
                    name = "test" } ]
          scriptFolders = folders
          excludeGlobPatterns = None
          embedded = FromConfig(cachedFiles, cached)

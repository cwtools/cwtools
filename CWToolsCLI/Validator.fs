...
module Validator =
    ...
    type STL (dir : string, scope : FilesScope, modFilter : string, triggers : DocEffect list, effects : DocEffect list) =
        ...
        let game = STLGame(dir, scope, modFilter, triggers, effects, [], [], [], langs, true, true, false)
        ...
        member val localisationErrorList = game.LocalisationErrors() |> List.map (fun (s,c,n,l,e,_) -> {category = s.GetType().Name ; error = e; position = n.ToString()})
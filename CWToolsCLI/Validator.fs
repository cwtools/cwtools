...
module Validator =
    ...
    type STL (dir : string, scope : FilesScope, modFilter : string, triggers : DocEffect list, effects : DocEffect list, config) =
        ...
        member __.validationErrorList() = game.ValidationErrors() |> List.map (fun e -> {category = e.code.GetType().Name ; error = e.message; position = e.range.ToString(); severity = e.severity})
        ...
        member __.localisationErrorList() = game.LocalisationErrors (true, true) |> List.map (fun e -> {category = e.code.GetType().Name ; error = e.message; position = e.range.ToString(); severity = e.severity})
